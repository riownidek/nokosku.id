...
// ─── Nav Item — uses Next.js <Link> natively, no router manipulation ─────────
function NavItem({
...
// ─── FAB (center button) — also plain <Link> ─────────────────────────────────
function FABItem({ href }: { href: string }) {
  return (
    <Link href={href} className="flex -mt-4 flex-col items-center" prefetch={true}>
      <motion.div
        whileTap={{ scale: 0.87 }}
